...
func TestPaymentResultStatusEndpoint(t *testing.T) {
	...
	for _, tx := range []*model.Transaction{
		...
		},
		{
			...
			UserID:             "user-1",
			Namespace:          "ns",
			Provider:           "generic_shopee_pay",
			CustomProviderName: "shopee_pay",
	...
	tests := []struct {
		...
		{"txn-pending", "PENDING", "DANA", "21.000 IDR"},
		{"txn-fulfilled", "FULFILLED", "DANA", "25.000 IDR"},
